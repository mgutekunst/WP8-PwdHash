         ...
         public static string Extract(string url)
		    {
			    ...
			    try
			    {
                    ...
                    {
                        var sld = mSecondLevelDomains.FirstOrDefault(host.EndsWith);
                        if(!string.IsNullOrEmpty(sld))
                        {
                            return string.Join(".",new []{splittedHost[splittedHost.Length - sld.Split('.').Length -1], sld});
                        }
                        {
                            return string.Join(".", splittedHost.Skip(splittedHost.Length - 2)); 
                        }
                    }
